...
OUT_DIR    = os.path.join(DATA_ROOT, "Predicted_Cortex_Masks")
...
def save_qc_png(img2d, lab2d, out_png):
    ...
    # right=blue, left=red (or swap if you prefer)
    rgb[right, 0] = 0
    rgb[right, 1] = 0
    rgb[right, 2] = 255
...
def predict_subject(subj_id: str, model: nn.Module, qc_slices: int = 3):
    ...
    out_path   = os.path.join(OUT_DIR,  f"{subj_id}_MOLLI_Native_cortex_pred.nii.gz")
...
def main():
    ...
    # Exclude the manual-labelled cortex cases
    manual_ids = set()
    manual_ids |= read_id_file(os.path.join(DATA_ROOT, "Cortex_UNet_3class", "train_ids_CORTEX.txt"))
    manual_ids |= read_id_file(os.path.join(DATA_ROOT, "Cortex_UNet_3class", "val_ids_CORTEX.txt"))
    manual_ids |= read_id_file(os.path.join(DATA_ROOT, "Cortex_UNet_3class", "test_ids_CORTEX.txt"))
    ...
    print("Manual cortex subjects:", len(manual_ids))
