...
OUTPUT_DIR = r"H:\Data\Kidney_Segmentation\Raw_Molli"
...
def find_molli_native_dicom(dicom_dir):
    ...
    for fname in os.listdir(dicom_dir):
        upper = fname.upper()
        if "MOLLI" in upper and "NATIVE" in upper:
            if "_SA" in upper or "SA_" in upper or "3SL" in upper:
                continue  
...
def process_patient(patient_dir):
    ...
    if molli_path is None:
        print(f"[SKIP] {patient_name}: no MOLLI Native DICOM found")
        return
    ...
    output_file = os.path.join(OUTPUT_DIR, f"{patient_name}_MOLLI_Native.nii.gz")
