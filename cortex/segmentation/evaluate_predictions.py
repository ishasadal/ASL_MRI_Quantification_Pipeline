...
OUT_FEATURES = os.path.join(DATA_ROOT, "cortex_segmentation_QC_features.csv")
OUT_CATA     = os.path.join(DATA_ROOT, "cortex_segmentation_QC_catastrophic_excluded.csv")
OUT_SUSP     = os.path.join(DATA_ROOT, "cortex_segmentation_QC_suspicious.csv")

PLOT_DIR = os.path.join(DATA_ROOT, "_QC_PLOTS_CORTEX_AUTO")
...
# Hard fails
MIN_TOTAL_VOX_HARD_Q  = 0.005  # bottom 0.5% => hard fail
MIN_KIDNEY_VOX_HARD_Q = 0.005
...
if len(paths) == 0:
    raise RuntimeError(f"No masks found in {MASK_DIR} matching pattern {PATTERN}")

records = []
for p in paths:
    base = os.path.basename(p)
    sid = base.replace("_MOLLI_Native_cortex_pred.nii.gz","").replace("_MOLLI_Native_cortex_pred.nii","")
...
# if one side is missing completely 
df.loc[df["right_vol"].fillna(0) == 0, "hard_fail"] = True
df.loc[df["left_vol"].fillna(0) == 0, "hard_fail"] = True

# quantile-based minimum volume thresholds 
ref = df.loc[(~df["hard_fail"]) &
             (df["right_vol"].fillna(0) > 0) &
             (df["left_vol"].fillna(0) > 0)].copy()

if len(ref) > 20:
    MIN_TOTAL_VOX_HARD  = float(ref["total_vol"].quantile(MIN_TOTAL_VOX_HARD_Q))
    MIN_KIDNEY_VOX_HARD = float(min(ref["right_vol"].quantile(MIN_KIDNEY_VOX_HARD_Q),
                                    ref["left_vol"].quantile(MIN_KIDNEY_VOX_HARD_Q)))
else:
    MIN_TOTAL_VOX_HARD  = 0.0
    MIN_KIDNEY_VOX_HARD = 0.0

print(f"[CORTEX] Hard thresholds (quantile-based): "
      f"MIN_TOTAL_VOX_HARD={MIN_TOTAL_VOX_HARD:.0f}, MIN_KIDNEY_VOX_HARD={MIN_KIDNEY_VOX_HARD:.0f}")
